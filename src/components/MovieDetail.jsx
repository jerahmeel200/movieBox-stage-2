...
export default function MovieDetail() {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/${id}?api_key=0b7942e48cf9befb7703ec2fe5626b3b&language=en-US&append_to_response=credits&append_to_response=videos`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/${id}?api_key=0b7942e48cf9befb7703ec2fe5626b3b&language=en-US&append_to_response=credits,videos`
    )
      .then((res) => res.json())
      .then((data) => {
        setMovieDetails(data);
        ...
        <div className="mainDetailSub">
          <div className="mainDetailSubTop">
            ...
            <div className="mainDetailSubTopRight">
              <img src={Star} /> <span className="rating">8.5</span>
              <strong> | 350k</strong>
            </div>
          </div>
          <div className="mainDetailSubBottom">
            <div className="mainDetailSubBottomLeft">
              ...
              <div>
                <p>
                  Director:{" "}
                  <span className="movieProducersInfo">
                    {getDirectorName(movieDetails.credits?.crew || [])}
                  </span>
                </p>
                <p>
                  Writer(s):{" "}
                  <span className="movieProducersInfo">
                    {getWritersNames(movieDetails.credits?.crew || [])}
                  </span>
                </p>
                <p>
                  Stars:{" "}
                  <span className="movieProducersInfo">
                    {getStarsNames(movieDetails.credits?.cast || [])}
                  </span>
                </p>
